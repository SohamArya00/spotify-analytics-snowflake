## Snowflake Implementation Screenshots

### Database Architecture
![Database Schemas](docs/docs/screenshots/snowflake_database_schemas.png)

### Raw Data Tables
![Raw Tables](docs/docs/screenshots/raw_tables_loaded.png)

### Curated Fact Table
![Curated Fact Table](docs/docs/screenshots/curated_fact_table.png)

### Analytics Views
![Top Tracks](docs/docs/screenshots/top_tracks_view.png)

![Skip Rate](docs/docs/screenshots/skip_rate_view.png)

### Data Quality Validation
![Data Quality Checks](docs/docs/screenshots/data_quality_checks.png)
